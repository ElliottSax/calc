...
import {
  ...
  Sparkles, ArrowRight, Bell, Bookmark, ExternalLink, Info
} from "lucide-react"
...
export function MarketInsights({ initialNews, newsSource, sourceNotice }: MarketInsightsProps) {
  ...
  const [newsItems, setNewsItems] = useState<NewsItem[]>(initialNews || defaultMockNews)
  const [dataSource, setDataSource] = useState<string>(newsSource || 'mock')

  // Refresh news periodically when live updates are enabled
  useEffect(() => {
  ...
  // Compute sentiment stats from current news
  const sentimentStats = (() => {
    ...
    return {
      ...
      bullish: Math.round((positive / total) * 100),
      neutral: Math.round((neutral / total) * 100),
      bearish: Math.round((negative / total) * 100),
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        {(sourceNotice || dataSource === 'curated') && (
          <div className="flex items-center gap-2 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
            <Info className="w-4 h-4 text-blue-400 flex-shrink-0" />
            <p className="text-sm text-blue-300">
              {sourceNotice || 'Showing curated dividend news. Connect a Finnhub API key for live market data.'}
            </p>
          </div>
        )}
      </motion.div>
                ...
                {filteredNews.length === 0 ? (
                  ...
                  >
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      className="mt-3 border-slate-600 text-gray-300"
          ...
          <Card className="bg-slate-800/50 backdrop-blur-sm border-slate-700">
            ...
            <CardContent className="space-y-4">
              <div className="text-center">
                <div className="text-4xl font-bold text-green-400 mb-2">{sentimentStats.score}</div>
                <div className="text-sm text-gray-400">Sentiment Score</div>
                <Progress value={sentimentStats.score} className="h-3 mt-2" />
              </div>

              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-300">Bullish News</span>
                  <span className="text-green-400">{sentimentStats.bullish}%</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-300">Neutral News</span>
                  <span className="text-gray-400">{sentimentStats.neutral}%</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-300">Bearish News</span>
                  <span className="text-red-400">{sentimentStats.bearish}%</span>
                </div>
              </div>

              <div className="pt-3 border-t border-slate-600">
                <div className="text-xs text-center text-gray-400">
                  Based on analysis of {sentimentStats.total} articles
                </div>
              </div>
            </CardContent>
          </Card>
