...
export function RelatedArticles({ currentSlug, articles }: { currentSlug: string; articles: Article[] }) {
  ...
  return (
    <section className="my-12 py-8 border-t border-gray-200">
      <h2 className="text-2xl font-bold mb-8">Related Strategies</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {related.map(a => (
          <Link key={a.slug} href={`/blog/${a.slug}`}>
            <div className="border border-gray-200 rounded-lg p-4 hover:shadow-lg hover:border-green-300 transition">
              <h3 className="font-bold text-lg mb-2 hover:text-green-600">{a.title}</h3>
